...
;;;; Preguntas del sistema a hacerle al usuario ;;;

(defmodule Preguntas
	(export deftemplate respuestas))
	...
	(declare (salience 9999))
	=>
	(printout t "Bienvenido al Sistema Experto Simple de Ruben Girela y Eduardo Morales" crlf)
	)
	...
	(defmodule Sruben
		...
		(defrule template_a_reglas
			(respuestas (like_mat ?a) (like_hardware ?b) (like_programming ?c) (trabajar ?d) (prefiere_t_p ?e) (like_work ?f) (Deduccion_nota_media ?g))
			=>
			(assert (like_mat ?a))
			(assert (like_hardware ?b))
			(assert (like_programming ?c))
			(assert (trabajar ?d))
			(assert (prefiere_t_p ?e))
			(assert (like_work ?f))
	...
	(defrule aconsejar
		...
		(test (neq ?Y DESCONOCIDO))
	=>
	(assert (ConsejoRuben (Rama ?X) (Explicacion ?Y) (Experto "Ruben Girela")))
	)
	...
	(defrule aconsejar2
		...
		(test (eq ?Y DESCONOCIDO))
	=>
	(assert (ConsejoRuben (Rama ?X) (Explicacion "Porque no te decides, deberias decidirte ;)")  (Experto "Ruben Girela")))
	)
